...
const renderTotal = (total?: number | totalType | React.ReactNode) => {
  ...
  switch (typeof total) {
    case 'undefined':
      ...
      break;
    case 'function':
      totalDom = <div className={styles.total} style={{width: '100%', textAlign: 'center' , fontWeight: 600}}>{total()}</div>;
      break;
    default:
      totalDom = <div className={styles.total} style={{width: '100%', textAlign: 'center', fontWeight: 600}}>{total}</div>;
  }
...
class ChartCard extends React.Component<ChartCardProps> {
  renderContent = () => {
    const { contentHeight, title, avatar, total, footer, children, loading } = this.props;
    if (loading) {
      return false;
    }
    return (
